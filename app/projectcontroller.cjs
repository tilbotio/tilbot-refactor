...
const { parseBrackets, isTrimmedBracketsEqual } = require('./parse-brackets.cjs');

const elseLabel = ['', ['else'], ''];
const andLabelPart = ['and'];

function splitAnds(label) {
  let conjunct = [];
  const result = [conjunct];
  for (let i = 0; i < label.length; i++) {
    const label_i = label[i];
    if (i & 1 && isTrimmedBracketsEqual(label_i, andLabelPart)) {
      conjunct = [];
      result.push(conjunct);
    } else {
      conjunct.push(label_i);
    }
  }
  return result;
}
...
class ProjectController {
  ...
  async find_matching_connector(block, str, else_output) {
    ...
    for (const connector of block.connectors) {
      ...
      if (isTrimmedBracketsEqual(label, elseLabel)) {
