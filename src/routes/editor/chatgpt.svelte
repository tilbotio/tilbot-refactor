...
<script lang="ts">
        ...
        if (typeof navigator === 'object' && typeof navigator.userAgent === 'string' && navigator.userAgent.indexOf('Electron') >= 0) {
            ...
            window.api.receive('csv-load', (param: any) => {
                if (is_loading_csv) {
                    ...
                    msgs = [{
                        role: "system",
                        content: settings.llm_prompt_sunny + "\n" + data_str
                    }];
    ...
    async function message_received(event: MessageEvent) {
        ...
        if (event.data.startsWith('Auto|')) {
            chatgpt_str += event.data.substr(5) + "\n\n";
        }
        else {
            chatgpt_str += event.data.substr(event.data.indexOf('|')+1);
