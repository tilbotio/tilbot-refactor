                    ...
                    {#each data.projects as p, index}
                      <tr>
                        ...
                        <td class="text-center">
                            <svg xmlns="http://www.w3.org/2000/svg" on:click={p.status?view_bot(p.id):undefined} fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="{p.status?'currentColor':'#e8e8e8'}" class="w-6 h-6 inline-block {p.status?'cursor-pointer':'disabled'}">
                        ...
                        <td class="text-center">
                            <svg on:click={get_logs(p.id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block cursor-pointer">
...
import { SvelteComponent, onMount } from 'svelte';
    ...
    function get_logs(id) {
        fetch(location.protocol + '//' + window.location.hostname + '/api/get_logs?projectid=' + id, {
                method: 'get',
                credentials: 'include'
            })
            .then(response => {
