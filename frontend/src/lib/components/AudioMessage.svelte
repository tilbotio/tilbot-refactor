  ...
  <div class="chat-bubble whitespace-pre-line bg-tilbot-secondary-hardpink">
    <div class="flex items-center mt-1" onclick={playStop}>
      {#if isPlaying}
        <Stop
          variation="solid"
          class="h-4 w-4 text-white cursor-pointer mr-2"
        />
      {:else}
        <Play
          variation="solid"
      ...
      <span class="text-xs text-gray-200"
        >{playingTimeToMinSecString(currentTime)}</span
      >
      <progress
        class="progress w-48 ml-4 mr-4"
        value={(Math.round(currentTime) / Math.round(duration)) * 100 || 0}
        max="100"
      ></progress>
      <span class="text-xs text-gray-200"
        >{playingTimeToMinSecString(duration)}</span
      >
    </div>
    {#if message.content != ""}
